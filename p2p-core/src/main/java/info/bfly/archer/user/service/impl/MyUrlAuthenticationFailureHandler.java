...
public class MyUrlAuthenticationFailureHandler extends SimpleUrlAuthenticationFailureHandler {
    ...
    public void onAuthenticationFailure(HttpServletRequest request, HttpServletResponse response, AuthenticationException exception) throws IOException, ServletException {
        ...
        String failTargetUrl = request.getParameter("spring-security-fail-redirect");
        if (StringUtils.hasText(failTargetUrl)) {
            saveException(request, exception);
            if (super.isUseForward()) {
                logger.debug("Forwarding to " + failTargetUrl);
                request.getRequestDispatcher(failTargetUrl).forward(request, response);
            }
            else {
                logger.debug("Redirecting to " + failTargetUrl);
                getRedirectStrategy().sendRedirect(request, response, failTargetUrl);
            }
        }
        else if (StringUtils.hasText(targetUrl) && useModalBox && targetUrl.endsWith("?needLogin=true")) {
            saveException(request, exception);
