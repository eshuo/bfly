...
public class CommentList extends EntityQuery<Comment> implements Serializable {
    private static final long serialVersionUID = 3966078480532085244L;
    private Date   searchcommitMinTime;
    private Date   searchcommitMaxTime;
    private String loanId;

    public CommentList() {
        final String[] RESTRICTIONS = {"loan.id = #{commentList.loanId}", "loan.id = #{commentList.example.loan.id}", "userByCreator.id = #{commentList.example.userByCreator.id}",
                "loan.name like #{commentList.example.loan.name}", "createTime >= #{commentList.searchcommitMinTime}", "createTime <= #{commentList.searchcommitMaxTime}"};
    ...
    private List<Comment> getChildren(Comment comment) {
        ...
        List<Comment> cComments = (List<Comment>) getHt().find(hql, comment.getThread() + "_%", comment.getNode().getId());
        if (cComments.size() != 0) {
     ...
     * @param nodeId
     * @param firstResult
     * @param maxResult   这里是指一级评论数量
     * @return
     */
    public List<Comment> getComments(String nodeId, int firstResult, int maxResult) {
     ...
     * @return
     */
    private List<Comment> getFLComments(String nodeId, int firstResult, int maxResult) {
        ...
        return (List<Comment>) getHt().findByCriteria(criteria, 0, maxResult);
    }
    ...
    public List<Comment> getLastestComments(int maxResults) {
        ...
        getHt().setCacheQueries(true);
        return (List<Comment>) getHt().findByCriteria(criteria, 0, maxResults);
    }
