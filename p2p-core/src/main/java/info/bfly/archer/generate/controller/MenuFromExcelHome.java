package info.bfly.archer.generate.controller;

import info.bfly.archer.generate.log.GenerateLog;
import info.bfly.archer.generate.model.MenuFromExcel;
...
public class MenuFromExcelHome {
    ...
    public static void main(String[] args) throws FileNotFoundException {
        String templateUrl = MenuFromExcelHome.class.getResource("内容填充模板.xlsx").getPath();
     ...
     * @param log
     */
    private Map<String, MenuFromExcel> buildTree(List<MenuFromExcel> menus, GenerateLog log) {
        ...
        for (MenuFromExcel menu : menus) {
            if (menusMap.containsKey(menu.getId())) {
                log.addErrorLog(menu.getLineNumber() + "与" + menusMap.get(menu.getId()).getLineNumber() + "id重复");
            }
            else {
        ...
        for (MenuFromExcel menu : menus) {
            if (StringUtils.isNotEmpty(menu.getPid())) {
                ...
                if (parent != null) {
                    ...
                    log.addErrorLog(menu.getLineNumber() + "找不到pid" + menu.getPid());
                }
            }
        }
        // 设置在树中的级别
            ...
            if (StringUtils.isEmpty(menu.getUrl())) {
                if (menu.getUrlType().equals("term")) {
                    ...
                    if (menu.getParent() != null) {
                        // 有父菜单，即该node有所属的term
                        menu.setUrl("/" + menu.getUrlType() + "/" + menu.getParent().getId() + "/" + menu.getId());
                    }
                    else {
     ...
     * @throws Exception
     */
    private void collectTypes(XSSFSheet typeSheet, List<TinyClass> termTypes, List<TinyClass> nodeTypes, List<TinyClass> urlTypes, List<TinyClass> menuTypes) throws Exception {
        for (int i = 2; i <= typeSheet.getLastRowNum(); i++) {
            ...
            if (StringUtils.isNotEmpty(getCellStringValue(row.getCell(0)))) {
                TinyClass termType = new TinyClass(getCellStringValue(row.getCell(0)), getCellStringValue(row.getCell(1)));
            ...
            if (StringUtils.isNotEmpty(getCellStringValue(row.getCell(2)))) {
                TinyClass nodeType = new TinyClass(getCellStringValue(row.getCell(2)), getCellStringValue(row.getCell(3)));
            ...
            if (StringUtils.isNotEmpty(getCellStringValue(row.getCell(4)))) {
                TinyClass urlType = new TinyClass(getCellStringValue(row.getCell(4)), getCellStringValue(row.getCell(5)));
            ...
            if (StringUtils.isNotEmpty(getCellStringValue(row.getCell(6)))) {
                TinyClass menuType = new TinyClass(getCellStringValue(row.getCell(6)), getCellStringValue(row.getCell(7)));
    ...
    private void createTerm(MenuFromExcel menu, Map<String, CategoryTerm> termsMap, List<CategoryTerm> terms) {
    ...
    public void generateMenu(InputStream is) {
        ...
        try {
            ...
            validateAndFillDefault(menus, generateLog, termTypes, nodeTypes, urlTypes, menuTypes);
            Map<String, MenuFromExcel> menusMap = buildTree(menus, generateLog);
            changeMenuUrl(menus);
            if (!generateLog.isEmpty()) {
                // 有错误，返回显示错误
                for (String error : generateLog.getErrorLogs()) {
                    log.info(error);
                }
            }
            else {
    ...
    private String getCellStringValue(XSSFCell cell) throws Exception {
        if (cell != null) {
            ...
            if (cell.getCellType() == Cell.CELL_TYPE_NUMERIC) {
                ...
                throw new Exception("单元格只支持数字和文本格式，请检查" + cell.getRow().getSheet().getSheetName() + " 行：" + (cell.getRowIndex() + 1) + "  列：" + (cell.getColumnIndex() + 1));
            }
        } else {
            return null;
        }
    ...
    private void insertMenus(List<MenuFromExcel> menus) {
        for (MenuFromExcel menu : menus) {
            String sql = "INSERT INTO menu " + "(id, description, enable, expanded, label, seq_num, url, pid, type, permission, target) " + "VALUES " + "('" + menu.getId() + "', #{description}, '"
                    + menu.getEnable() + "', '" + menu.getExpanded() + "', " + "'" + menu.getLabel() + "', " + menu.getSeqNum() + ", '" + menu.getUrl() + "', #{pid}, '" + menu.getType()
    ...
    private void insertMenuTypes(List<TinyClass> menuTypes) {
        for (TinyClass menuType : menuTypes) {
            String sql = "INSERT INTO menu_type " + "(id, name, description, enable) " + "VALUES " + "('" + menuType.getId() + "', '" + menuType.getName() + "', null, '1');";
            log.info(sql);
        }
    }
    ...
    private void insertNodeTerm(List<CategoryTerm> terms) {
        for (CategoryTerm term : terms) {
            String sql = "INSERT INTO category_term_node " + "(node_id, term_id) " + "VALUES " + "('test', '" + term.getId() + "');";
            log.info(sql);
        }
    }
    ...
    private void insertNodeTypes(List<TinyClass> nodeTypes) {
        for (TinyClass nodeType : nodeTypes) {
            String sql = "INSERT INTO node_type " + "(id, name, description) " + "VALUES " + "('" + nodeType.getId() + "', '" + nodeType.getName() + "', null);";
            log.info(sql);
        }
    }
    ...
    private void insertSingleNode(List<MenuFromExcel> menus) {
        for (MenuFromExcel menu : menus) {
            if (menu.getUrlType().equals("node")) {
                String hqlNodeBody = "INSERT INTO `node_body` (id,body) VALUES ('" + menu.getId() + "_node_body" + "', '" + menu.getLabel() + "');";
                ...
                log.info(hqlNodeBody);
                log.info(hqlNode);
                if (menu.getParent() == null) {
                    // 所有属于单页的node，都归属于single_page分类
                    String sql = "INSERT INTO category_term_node " + "(node_id, term_id) " + "VALUES " + "('" + menu.getId() + "', 'single_page');";
                    log.info(sql);
                }
                else {
                    String sql = "INSERT INTO category_term_node " + "(node_id, term_id) " + "VALUES " + "('" + menu.getId() + "', '" + menu.getParent().getId() + "');";
                    log.info(sql);
                }
            }
        }
    ...
    private void insertTerms(List<CategoryTerm> terms) {
        for (CategoryTerm term : terms) {
            String sql = "INSERT INTO category_term " + "(id, name, seq_num, type, pid) " + "VALUES " + "('" + term.getId() + "', '" + term.getName() + "', " + term.getSeqNum() + ", '"
        ...
        // 插入单页分类
        String sql = "INSERT INTO category_term " + "(id, name, seq_num, type, pid) " + "VALUES " + "('single_page', '单页', 1024, 'article', null);";
        log.info(sql);
    }

    private void insertTermTypes(List<TinyClass> termTypes) {
        for (TinyClass termType : termTypes) {
            String sql = "INSERT INTO category_term_type " + "(id, name, description) " + "VALUES " + "('" + termType.getId() + "', '" + termType.getName() + "', null);";
            log.info(sql);
        }
    }
    ...
    private List<MenuFromExcel> sortBySeqNum(List<MenuFromExcel> menus) {
        Collections.sort(menus, new Comparator<MenuFromExcel>() {
            ...
            public int compare(MenuFromExcel o1, MenuFromExcel o2) {
                return Integer.parseInt(o1.getSeqNum()) - Integer.parseInt(o2.getSeqNum());
            }
        });
        return menus;
    }
     ...
     * @param termTypes
     */
    private void validateAndFillDefault(List<MenuFromExcel> menus, GenerateLog log, List<TinyClass> termTypes, List<TinyClass> nodeTypes, List<TinyClass> urlTypes, List<TinyClass> menuTypes) {
        for (MenuFromExcel menu : menus) {
            // 验证Id
            ...
            else if (!menu.getEnable().equals("1") && !menu.getEnable().equals("0")) {
            ...
            else if (!menu.getExpanded().equals("1") && !menu.getExpanded().equals("0")) {
...
class TinyClass {
    ...
    public boolean equals(Object obj) {
        if (obj != null) {
            ...
            if (o2.getId() == getId() || o2.getId().equals(getId())) {
                return true;
            }
        }
        return false;
