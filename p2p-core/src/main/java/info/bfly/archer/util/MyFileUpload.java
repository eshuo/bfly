...
public class MyFileUpload {
    public static final Logger log = LoggerFactory.getLogger(MyFileUpload.class);
    public static void main(String[] args) {
        MyFileUpload.upLoad1();
    }
    ...
    public static void upLoad1() {
        ...
        // class文件
        String[] classPath = {
                // "info.bfly.archer.user.controller.investorBillList",
                // "info.bfly.archer.user.controller.loanerBillList",
                // "info.bfly.archer.user.service.LoanerBillService",
                // "info.bfly.archer.user.service.impl.LoanerBillServiceImpl"
        };
        // sqlMap.xml文件
        String[] xmlPath = {};
        ...
        // 其他xhtml、js文件等
        String[] otherPath = {"\\admin\\banner\\bannerEdit.xhtml",
        ...
        if (classPath.length > 0) {
            ...
            for (String string : classPath) {
                ...
                // 末尾加上.class
                string += ".class";
                up.writeFile(path1 + "WEB-INF\\classes\\" + string, localpath + date + "\\" + path + "\\WEB-INF\\classes\\" + string.substring(string.indexOf("\\"), string.length()));
            }
            log.info("class文件拷贝完毕！");
        }
        if (xmlPath.length > 0) {
            log.info("开始拷贝sqlMap.xml文件");
            // 拷贝sqlMap.xml文件
            for (String string : xmlPath) {
                string = up.str_replace("/", "\\", string);
                up.writeFile(path1 + "WEB-INF\\classes\\" + string, localpath + date + "\\" + path + "WEB-INF\\classes\\" + string.substring(string.indexOf("\\"), string.length()));
            }
            log.info("sqlMap.xml文件拷贝完毕！");
        }
        if (confPath.length > 0) {
            log.info("开始拷贝conf配置文件");
            for (String string : confPath) {
                string = up.str_replace("/", "\\", string);
                up.writeFile(path1 + "WEB-INF\\" + string, localpath + date + "\\" + path + "WEB-INF\\" + string.substring(string.indexOf("\\"), string.length()));
            }
            log.info("conf配置文件拷贝完毕！");
        }
        if (otherPath.length > 0) {
            log.info("开始拷贝jsp、js等文件");
            for (String string : otherPath) {
                string = up.str_replace("/", "\\", string);
                // dqmhA20130314_war.ear
                up.writeFile(path1 + string, localpath + date + "\\" + path + string.substring(string.indexOf("\\"), string.length()));
            }
            log.info("jsp、js等文件拷贝完毕！");
        }
    }
    ...
    public Object[] writeFile(String in_file_path, String fileAllPath) {
        if (in_file_path.indexOf("*") != -1) {
            ...
            for (String name : name_list) {
                url_n2 = url_n + name;
                if (url_n2.indexOf(".") != -1 && url_n2.substring(url_n2.lastIndexOf("\\") - 1, url_n2.length()).indexOf(".") != -1) {
                    writeFile2(url_n2, fileAllPath.substring(0, fileAllPath.lastIndexOf("\\") + 1) + name);
                }
            }
            return null;
        } else {
            writeFile2(in_file_path, fileAllPath);
        }
        return null;
    }
    ...
    public Object[] writeFile2(String in_file_path, String fileAllPath) {
        ...
        String WEB_PATH = fileAllPath.substring(0, fileAllPath.lastIndexOf("\\"));
        try {
