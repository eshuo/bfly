...
import info.bfly.p2p.schedule.job.CheckInvestTransferOverExpectTime;
...
public class TransferServiceImpl implements TransferService {
    ...
    public void applyInvestTransfer(TransferApply ta) throws ExceedInvestTransferMoney {
        ...
        if (ArithUtil.add(-investRRM.getUnPaidInterest(), investRRM.getUnPaidFee()) > ta.getPremium() || ta.getPremium() > ArithUtil.sub(investRRM.getUnPaidCorpus(), calculateFee(ta))) {
            throw new ExceedInvestTransferMoney("折让金区间不正确");
        }
        if (ta.getPremium() > 0) {
            String canBigger = configService.getConfigValue(ConfigConstants.InvestTransfer.CAN_GREATER_THAN_SELF_WORTH);
            if (!"1".equals(canBigger)) {
                throw new ExceedInvestTransferMoney("转让金额不能大于可转让金额");
            }
        }
        else if (ta.getPremium() < 0) {
            String canLess = configService.getConfigValue(ConfigConstants.InvestTransfer.CAN_LESS_THAN_SELF_WORTH);
            if (!"1".equals(canLess)) {
            ...
            String canEquel;
            try {
                canEquel = configService.getConfigValue(ConfigConstants.InvestTransfer.CAN_EQUAL_SELF_WORTH);
            }
        ...
        String timelimit;
        try {
            timelimit = configService.getConfigValue(ConfigConstants.InvestTransfer.DEAD_LINE);
        }
        catch (ObjectNotFoundException onfe) {
            timelimit = "7";
        }
        ...
        JobDetail jobDetail = JobBuilder.newJob(CheckInvestTransferOverExpectTime.class).withIdentity(ta.getId(), ScheduleConstants.JobGroup.CHECK_INVEST_TRANSFER_OVER_EXPECT_TIME).build();// 任务名，任务组，任务执行类
        jobDetail.getJobDataMap().put(CheckInvestTransferOverExpectTime.INVEST_TRANSFER_ID, ta.getId());
        SimpleTrigger trigger = TriggerBuilder.newTrigger().withIdentity(ta.getId(), ScheduleConstants.TriggerGroup.CHECK_INVEST_TRANSFER_OVER_EXPECT_TIME).forJob(jobDetail)
                .withSchedule(SimpleScheduleBuilder.simpleSchedule()).startAt(ta.getDeadline()).build();
        try {
    ...
    public double calculateFee(TransferApply ta) {
        return feeConfigBO.getFee(FeePoint.TRANSFER, FeeType.FACTORAGE, null, null, ta.getCorpus());
    }
    ...
    public double calculateInvestTransferCompletedRate(String transferApplyId) throws NoMatchingObjectsException {
        ...
        return ArithUtil.sub(1, ArithUtil.div(calculateRemainCorpus(ta.getId()), ta.getCorpus(), 4)) * 100;
    }
    ...
    public double calculateRemainCorpus(String transferApplyId) {
        ...
        for (Invest i : ta.getInvests()) {
            if (!InvestStatus.CANCEL.equals(i.getStatus())) {
                transferedCorpus = ArithUtil.add(i.getInvestMoney(), transferedCorpus);
            }
        }
    ...
    public String calculateRemainTime(String transferApplyId) throws NoMatchingObjectsException {
        TransferApply ta = ht.get(TransferApply.class, transferApplyId);
        if (ta == null) {
            throw new NoMatchingObjectsException(Loan.class, "transferApplyId:" + transferApplyId);
        }
    ...
    public double calculateWorth(String investId, double corpus) {
        ...
        for (int i = 0; i < irsOri.size(); i++) {
            ...
            if (irTemp.getStatus().equals(RepayStatus.REPAYING)) {
                ...
                Date prevRepayDay;
                if (i == 0) {
                    prevRepayDay = irTemp.getInvest().getLoan().getInterestBeginTime();
                } else {
                    prevRepayDay = irsOri.get(i - 1).getRepayDay();
                }
                daysM = DateUtil.getIntervalDays(prevRepayDay, irTemp.getRepayDay());
                if (prevRepayDay.before(new Date())) {
                    daysPassedM = DateUtil.getIntervalDays(prevRepayDay, new Date());
                } else {
                    daysPassedM = 0;
                }
                unPaidCorpus = irTemp.getInvest().getRepayRoadmap().getUnPaidCorpus();
                period = irTemp.getPeriod();
                break;
            }
        }
        String hql = "from InvestRepay ir where ir.invest.transferApply.invest.id=? and ir.period=?";
        List<InvestRepay> irs = (List<InvestRepay>) ht.find(hql, investId, period);
    ...
    public boolean canTransfer(String investId) throws InvestTransferException {
        ...
        List<TransferApply> tas = Lists.newArrayList(Collections2.filter(invest.getTransferApplies(), new Predicate<TransferApply>() {
            @Override
            public boolean apply(TransferApply transferApply) {
                return transferApply.getStatus().equals(TransferStatus.TRANSFERING) || transferApply.getStatus().equals(TransferStatus.WAITCONFIRM);
            }
        }));
        if (tas.size() > 0) {
            return false;
        ...
        try {
            // 债权的最少持有时间
            paidRepayMin = configService.getConfigValue(ConfigConstants.InvestTransfer.PAID_REPAY_COUNT_MIN);
        }
        catch (ObjectNotFoundException onfe) {
            // 默认值为1
            paidRepayMin = "1";
        ...
        try {
            remainRepayCountMin = configService.getConfigValue(ConfigConstants.InvestTransfer.REMAIN_REPAY_COUNT_MIN);
        }
        catch (ObjectNotFoundException onfe) {
            remainRepayCountMin = "3";
        }
        if (invest.getRepayRoadmap().getUnPaidPeriod() < Integer.valueOf(remainRepayCountMin)) {
            // 不合格
            return false;
        }
        // 剩余本金大于或等于?元。
        String remainCorpus;
        try {
            remainCorpus = configService.getConfigValue(ConfigConstants.InvestTransfer.REMAIN_CORPUS_MIN);
        }
        catch (ObjectNotFoundException onfe) {
            remainCorpus = "1000";
        }
        if (invest.getRepayRoadmap().getUnPaidCorpus() < Integer.valueOf(remainCorpus)) {
            // 不合格
            return false;
        }
        ...
        String deadLine;
        try {
            deadLine = configService.getConfigValue(ConfigConstants.InvestTransfer.APPLY_BEFORE_REPAY_DAY);
        }
        catch (ObjectNotFoundException onfe) {
            deadLine = "7";
        }
        ...
        return !newDate.after(invest.getRepayRoadmap().getNextRepayDate());
    }
    ...
    @Override
    @Transactional(rollbackFor = Exception.class)
    public void transfer(String transferApplyId, String userId, double transferCorpus) throws InsufficientBalance, ExceedInvestTransferMoney {
        ...
        if (transferCorpus <= 0 || transferCorpus > remainCorpus) {
            throw new ExceedInvestTransferMoney("购买本金必须小于等于" + remainCorpus + "且大于0");
        }
        TransferApply ta = ht.get(TransferApply.class, transferApplyId);
        ht.evict(ta);
        ta = ht.get(TransferApply.class, transferApplyId, LockMode.PESSIMISTIC_WRITE);
        // 购买的本金占所有转让本金的比例。
        ...
        ht.save(investNew);
        // 减去invest中持有的本金
        ta.getInvest().setMoney(ArithUtil.sub(ta.getInvest().getMoney(), transferCorpus));
        ...
        userBillBO.transferOutFromBalance(userId, buyPrice, OperatorInfo.TRANSFER, "购买债权，编号：" + investNew.getId());
        ...
        sbs.transferInto(fee, OperatorInfo.TRANSFER, "购买债权手续费，编号：" + investNew.getId());
        userBillBO.transferIntoBalance(ta.getInvest().getUser().getId(), buyPrice, OperatorInfo.TRANSFER, "债权转让成功，编号：" + transferApplyId);
        userBillBO.transferOutFromBalance(ta.getInvest().getUser().getId(), fee, OperatorInfo.TRANSFER, "债权转让成功手续费，编号：" + transferApplyId);
        ...
        for (Iterator iterator = ta.getInvest().getInvestRepays().iterator(); iterator.hasNext(); ) {
            ...
            if (ir.getStatus().equals(RepayStatus.WAIT_REPAY_VERIFY) || ir.getStatus().equals(RepayStatus.OVERDUE) || ir.getStatus().equals(RepayStatus.BAD_DEBT)) {
                throw new RuntimeException("investRepay with status " + RepayStatus.WAIT_REPAY_VERIFY + "exist!");
