...
import info.bfly.p2p.risk.FeeConfigConstants.OperateMode;
import info.bfly.p2p.risk.exception.NotContinuityIntervalException;
...
public class FeeConfigBO {
     ...
     * @return
     */
    private DetachedCriteria createDetachedCriteriaByCompositeKeys(String feePoint, String feeType, String factor, String factorValue) {
     ...
     *
     * @param fcId 需要被删除的费率规则（FeeConfig）id
     * @throws NoMatchingObjectsException     对象未找到
     * @throws NotContinuityIntervalException 删除后区间不连续
     */
    @Transactional(readOnly = false, rollbackFor = Exception.class)
    public void delete(String fcId) throws NoMatchingObjectsException, NotContinuityIntervalException {
        FeeConfig fc = ht.get(FeeConfig.class, fcId);
        if (fc == null) {
            throw new NoMatchingObjectsException(FeeConfig.class, fcId);
        }
        DetachedCriteria criteria = createDetachedCriteriaByCompositeKeys(fc.getFeePoint(), fc.getFeeType(), fc.getFactor(), fc.getFactorValue());
        List<FeeConfig> fcs = (List<FeeConfig>) ht.findByCriteria(criteria);
        // 并移除需要删除的对象
        remove(fc.getId(), fcs);
        if (fcs.size() >= 2) {
            // 判断是否连续
     ...
     * @since 2.0
     */
    public double getFee(String feePoint, String feeType, String factor, String factorValue, double value) {
        DetachedCriteria criteria = createDetachedCriteriaByCompositeKeys(feePoint, feeType, factor, factorValue);
        // value大于等于下限，小于上限
        criteria.add(Restrictions.and(Restrictions.gt("intervalUpperLimit", value), Restrictions.le("intervalLowerLimit", value)));
        List<FeeConfig> fcs = (List<FeeConfig>) ht.findByCriteria(criteria);
        if (fcs.size() > 1) {
            // 找到多个，抛异常。
            throw new DuplicateKeyException("feePoint:" + feePoint + " feeType:" + feeType + " factor:" + factor + " factorValue:" + factorValue + " value:" + value + "  duplication!");
        }
        if (fcs.size() == 0) {
            throw new ObjectNotFoundException(FeeConfig.class, "feePoint:" + feePoint + " feeType:" + feeType + " factor:" + factor + " factorValue:" + factorValue + " value:" + value
                    + "  not found!");
        }
        ...
        if (fc.getOperateMode().equals(OperateMode.FIXED)) {
            ...
            return Doubles.min(fee, fc.getFeeUpperLimit() == null ? fee : fc.getFeeUpperLimit());
        }
        else {
            throw new RuntimeException("operateMode: " + fc.getOperateMode() + "not found!");
        }
    }
     ...
     *             区间不连续
     */
    private void isContinuityInterival(List<FeeConfig> fcs) throws NotContinuityIntervalException {
        for (int i = 0; i <= fcs.size() - 2; i++) {
            if (fcs.get(i).getIntervalUpperLimit() != fcs.get(i + 1).getIntervalLowerLimit()) {
                // 不连续，则抛异常
     ...
     */
    @Transactional(readOnly = false, rollbackFor = Exception.class)
    public void saveOrUpdate(FeeConfig fc) throws NotContinuityIntervalException {
        DetachedCriteria criteria = createDetachedCriteriaByCompositeKeys(fc.getFeePoint(), fc.getFeeType(), fc.getFactor(), fc.getFactorValue());
        List<FeeConfig> fcs = (List<FeeConfig>) ht.findByCriteria(criteria);
    ...
    public void sortByIntervalLowerLimitAsc(List<FeeConfig> fcs) {
        Collections.sort(fcs, new Comparator<FeeConfig>() {
            ...
            public int compare(FeeConfig o1, FeeConfig o2) {
                double result = o1.getIntervalLowerLimit() - o2.getIntervalLowerLimit();
                if (result > 0) {
                    return 1;
                }
                else if (result < 0) {
