...
public class MyPrettyFilter implements Filter {
    ...
    public static final  String CONFIG_KEY              = "info.bfly.urlMapping.CONFIG_FILES";
    ...
    public void doFilter(final ServletRequest req, final ServletResponse resp, final FilterChain chain) throws IOException, ServletException {
        ...
        HttpServletResponse response = new PrettyFacesWrappedResponse(request.getContextPath(), request, (HttpServletResponse) resp, getConfig(request));
            ...
            if (mapping != null) {
                ...
                if (!response.isCommitted()) {
                    if (context.shouldProcessDynaview()) {
                        ...
                        log.trace("Sending mapped request [" + url.toURL() + "] to model [" + viewId + "]");
                        if (url.decode().toURL().equals(viewId)) {
                            chain.doFilter(wrappedRequest, response);
                        } else {
                            setUrlMappingForward(request);
                            if (viewId.contains("#{pageName}")) {
                                viewId = viewId.replace("#{pageName}", wrappedRequest.getParameter("pageName"));
                            }
    ...
    public PrettyConfig getConfig(HttpServletRequest hreq) {
        ...
        PrettyConfig pc = (PrettyConfig) hreq.getSession(true).getServletContext().getAttribute(CONFIG_KEY);
        ...
        if (pc == null) {
            ...
            pc = (PrettyConfig) servletContext.getAttribute(PrettyContext.CONFIG_KEY);
            pc = handleUserConfig(pc, hreq);
            // hreq.getSession(true).setAttribute(PrettyContext.CONFIG_KEY, pc);
            hreq.getSession(true).getServletContext().setAttribute(PrettyContext.CONFIG_KEY, pc);
        }
        return pc;
    }
    ...
    private PrettyConfig handleUserConfig(PrettyConfig pc, HttpServletRequest hreq) {
        String userTheme = (String) hreq.getSession(true).getAttribute(USER_THEME);
        if (userTheme == null) {
            // 如果用户的主题不存在，取默认主题
