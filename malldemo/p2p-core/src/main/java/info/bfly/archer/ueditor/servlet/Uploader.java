...
public class Uploader {
    ...
    public void upload() throws Exception {
        ...
        if (!isMultipart) {
            state = errorInfo.get("NOFILE");
            return;
        }
        if (request instanceof MultipartRequest) {
            DiskFileItem dfi = (DiskFileItem) ((MultipartRequest) request).getFileItem("upfile");
            title = request.getParameter("pictitle");
            originalName = dfi.getName().substring(dfi.getName().lastIndexOf(System.getProperty("file.separator")) + 1);
